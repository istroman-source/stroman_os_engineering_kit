...
const inputClass = "flex-1 rounded-md border border-border bg-background px-3 py-2 text-sm outline-none focus-visible:ring-2 focus-visible:ring-ring";

function statusLabel(location: PreparedLocationItem): string {
  if (location.status === "READY") return "Ready to use";
  if (location.status === "FAILED" || location.status === "NEEDS_ATTENTION") return "Needs attention";
...
export function LocationsView() {
  ...
  useEffect(() => {
    let active = true;
    listPreparedLocations().then((items) => active && setLocations(items)).catch((err) => {
      if (!active) return;
  ...
  return <section className="border-border bg-card flex flex-col gap-4 rounded-lg border p-5" aria-label="Locations">
    <div>
      ...
      <p className="text-muted-foreground text-sm">Prepare real spaces once, then use them when you start a story.</p>
    </div>
    <form onSubmit={onCreate} className="flex flex-col gap-3">
      <input className={inputClass} value={name} onChange={(event) => setName(event.target.value)} placeholder="Location name — e.g. Downtown kitchen" maxLength={160} aria-label="Location name" />
      <div className="flex flex-wrap gap-2" aria-label="Room input method">
        <Button type="button" variant={inputKind === "PHOTOS" ? "default" : "outline"} onClick={() => setInputKind("PHOTOS")}>Build from photos</Button>
        <Button type="button" variant={inputKind === "GLB" ? "default" : "outline"} onClick={() => setInputKind("GLB")}>Upload a 3D scan</Button>
        <Button type="submit" disabled={busy || !name.trim()}>{busy ? "Adding…" : "Add location"}</Button>
      </div>
    </form>
