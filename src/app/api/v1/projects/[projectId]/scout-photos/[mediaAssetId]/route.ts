...
export const GET = apiRoute<{ projectId: string; mediaAssetId: string }>(
  async ({ req, params }) => {
    ...
    if (!brief?.planningContext.scoutPhotos.some((photo) => photo.mediaAssetId === mediaAssetId)) {
      throw new HttpError(404, "NOT_FOUND", "Scout photo not found.");
    }
    const receipt = (await context.sourceImports.listByProject(projectId)).find(
      (item) => item.mediaAssetId === mediaAssetId,
    );
    if (!receipt) throw new HttpError(404, "NOT_FOUND", "Scout photo not found.");
