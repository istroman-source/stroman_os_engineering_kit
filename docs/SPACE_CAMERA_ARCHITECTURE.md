...
The photo-to-room implementation now makes the adapter coordinate contract executable rather than documentary. A provider result supplies an axis-only source-to-canonical transform plus an optional trustworthy source-unit scale. Stroman composes that with inferred scale, transforms the GLB bounds, persists the exact matrix and applies that same matrix to the immutable renderer asset. The deterministic equivalence test imports the same room through both a meter/Y-up adapter and a centimeter/Z-up adapter; both produce identical canonical bounds and initial 16:9/9:16 filmmaking cameras while retaining different source transforms. This closes the provider-neutral coordinate proof, but does not substitute for the outstanding live real-location evidence above.
